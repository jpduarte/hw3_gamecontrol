...
class Test
{
    ...
    {   //define class to later call functions
        Test machinelearning_mat = new Test();
        int threshold_data = 50000;
        string line;
        float time;
        float[] add_data_time = new float[44];
        float[] projection1_time = new float[44];
        float mean_time_data;
        for (int i=0; i<44;i++){
         add_data_time[i]=0;
         projection1_time[i]=0;
        }
        float distance;
        ...
        /////////////////////////////////////////////////enter all data from mat and MAIN CALCULATIONS
        ...
        while((line = file1.ReadLine()) != null)
         {
             ...
             if (add_data>threshold_data) {//check threshold data
              projections = machinelearning_mat.PCA_classify(mat_data,basis,mean_vector,projections);
              state = machinelearning_mat.which_cluster(projections,centroids);
             } else {
              state=-1;
             }
             ...
             add_data_time = machinelearning_mat.FIFO_update(add_data_time,add_data);
             mean_time_data = machinelearning_mat.mean_data(add_data_time);
             distance = machinelearning_mat.distance_to_template(add_data_time,mean_time_data,polynomial_squad);
             Console.WriteLine(distance);
             if ((distance)<4.267e14){
              Console.WriteLine("squad squad squad squad squad squad squad");
             }

             //detect push up, this is like a switch data, so a positive must be handle like such, i.e. many positives will be together
             projection1_time = machinelearning_mat.FIFO_update(projection1_time,projections[0]);
             mean_time_data = machinelearning_mat.mean_data(projection1_time);
             distance = machinelearning_mat.distance_to_template(projection1_time,mean_time_data,polynomial_push_up);
             if ((distance)<7483147950.81){
              Console.WriteLine("push-up push-up push-up push-up push-up push-up ");
             }
