...
for i in range(4,len(time)):
    #plotelement = -np.sum(datalist[i,1:])+8*np.sum(datalist[i-1,1:])+0*np.sum(datalist[i-2,1:])-8*np.sum(datalist[i-3,1:])+np.sum(datalist[i-4,1:])

    plotelement.append((np.sum(datalist[i,1:])+np.sum(datalist[i-1,1:])+np.sum(datalist[i-2,1:])+np.sum(datalist[i-3,1:])+np.sum(datalist[i-4,1:]))/5.0)
